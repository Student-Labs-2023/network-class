...
from src.error_codes import ERROR_CODE_USER_NOT_AUTHORIZED
...
token_auth0_scheme = HTTPBearer()


async def check_auth(token: str = Depends(token_auth0_scheme)):
    result = VerifyToken(token.credentials).verify()

    if result.get("status"):
        raise HTTPException(status_code=ERROR_CODE_USER_NOT_AUTHORIZED, detail=result)